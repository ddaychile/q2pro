...
gl_profile::
    ...
    profile context. On macOS, the default is "gl4.5" (core profile). Can be
    set to "gl" to create OpenGL core profile context, or "es" to create
    OpenGL ES context. May be followed by desired minimum OpenGL version
    number. For example, "gl4.6" or "es3.0". Not all OpenGL implementations
    support this.
